 ...
 *     DCS (Dynamic Channel Selection) configuration parameters. This enables or
...
enum qca_wlan_vendor_attr_ll_stats_results {
	 ...
	 * by firmware and reported via the nested attributes defined in
 ...
 * by target hardware. The bin configuration (range and step) is dynamic and
 ...
 *	is lesser than this threshold. The RSSI threshold through this attribute
 ...
 *
 * @QCA_WLAN_VENDOR_FEATURE_SUPPORT_USER_SCENARIO_TO_DSI_MAPPING: Flag
 * indicates that the driver supports enabling the user scenario to DSI index
 * mapping.
 *
...
/**
 * enum qca_wifi_power_scenario - Represents different power scenarios that
 * can be configured through %QCA_WLAN_VENDOR_ATTR_CONFIG_POWER_SCENARIO.
 *
 * @QCA_WIFI_POWER_SCENARIO_VOICE_CALL: Power scenario optimized for voice
 * calls, balancing connectivity with reduced interference to voice
 * communications.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_HEAD_CELL_OFF: Power settings for device held
 * near head with cellular radio disabled, optimizing WiFi transmission power
 * to minimize SAR exposure while maintaining connectivity.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_HEAD_CELL_ON: Power settings for device held near
 * head with cellular radio enabled, managing combined RF exposure from WiFi and
 * cellular radios to comply with regulatory SAR limits.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_BODY_CELL_OFF: Power settings for device carried
 * on body with cellular radio disabled, optimized for body-worn position while
 * ensuring regulatory compliance for body SAR limits.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_BODY_CELL_ON: Power settings for device carried
 * on body with cellular radio enabled, balancing WiFi performance with cellular
 * coexistence in body-worn position.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_BODY_BT: Power settings for device carried on
 * body with Bluetooth active, optimizing WiFi and Bluetooth coexistence in
 * body-worn position.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_HEAD_HOTSPOT: Power settings for device used as
 * hotspot while held near head, optimizing tethering functionality while
 * managing RF exposure near the user's head.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_HEAD_HOTSPOT_MMW: Power settings for device used
 * as hotspot near head with millimeter wave technology active, with special
 * power settings for high-frequency operation near head.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_BODY_CELL_ON_BT: Power settings for device on
 * body with both cellular and Bluetooth active, managing triple-radio
 * coexistence in body-worn position.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_BODY_HOTSPOT: Power settings for device used as
 * hotspot while carried on body, optimizing tethering performance in body-worn
 * position.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_BODY_HOTSPOT_BT: Power settings for device used
 * as hotspot on body with Bluetooth active, balancing hotspot and Bluetooth
 * coexistence in body-worn position.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_BODY_HOTSPOT_MMW: Power settings for device used
 * as hotspot on body with mmWave technology, optimizing high-frequency hotspot
 * operation in body-worn position.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_BODY_HOTSPOT_BT_MMW: Power settings for device
 * used as hotspot on body with both Bluetooth and mmWave active, managing
 * complex triple-technology coexistence in body-worn position.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_HEAD_CELL_OFF_UNFOLDED: Power settings for
 * foldable device in unfolded state, held near head with cellular radio
 * disabled, optimizing WiFi power for larger form factor near head.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_HEAD_CELL_ON_UNFOLDED: Power settings for
 * foldable device in unfolded state, held near head with cellular radio
 * enabled, managing combined RF exposure in unfolded configuration near head.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_HEAD_HOTSPOT_UNFOLDED: Power settings for
 * foldable device in unfolded state, used as hotspot near head, optimizing
 * tethering performance for unfolded configuration.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_HEAD_HOTSPOT_MMW_UNFOLDED: Power settings for
 * foldable device in unfolded state, used as mmWave hotspot near head, with
 * special power settings for high-frequency tethering in unfolded
 * configuration.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_BODY_CELL_OFF_UNFOLDED: Power settings for
 * foldable device in unfolded state, carried on body with cellular radio
 * disabled, optimizing WiFi power for unfolded body-worn position.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_BODY_BT_UNFOLDED: Power settings for foldable
 * device in unfolded state, carried on body with Bluetooth active, managing
 * WiFi-Bluetooth coexistence in unfolded body-worn position.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_BODY_CELL_ON_UNFOLDED: Power settings for
 * foldable device in unfolded state, carried on body with cellular radio
 * enabled, balancing WiFi and cellular performance in unfolded body-worn
 * position.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_BODY_CELL_ON_BT_UNFOLDED: Power settings for
 ...
 * configuration.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_BODY_HOTSPOT_UNFOLDED: Power settings for
 * foldable device in unfolded state, used as hotspot while on body, optimizing
 * tethering performance in unfolded body-worn position.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_BODY_HOTSPOT_BT_UNFOLDED: Power settings for
 * foldable device in unfolded state, used as hotspot on body with Bluetooth
 * active, balancing hotspot and Bluetooth operation in unfolded body-worn
 * position.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_BODY_HOTSPOT_MMW_UNFOLDED: Power settings for
 * foldable device in unfolded state, used as mmWave hotspot on body,
 * optimizing high-frequency tethering in unfolded body-worn position.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_BODY_HOTSPOT_BT_MMW_UNFOLDED: Power settings for
 * foldable device in unfolded state, used as hotspot on body with both
 * Bluetooth and mmWave active, managing complex triple-technology coexistence
 * in unfolded body-worn position.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_BODY_REAR_CAMERA: Power settings for device on
 * body with rear camera active, optimizing WiFi power to minimize interference
 * with camera operations while maintaining connectivity.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_BODY_CELL_OFF_UNFOLDED_CAP: Power settings for
 * foldable device with capacitive touch in unfolded state, on body with
 * cellular radio disabled, optimizing WiFi power while ensuring touch
 * sensitivity is maintained.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_BODY_BT_UNFOLDED_CAP: Power settings for
 * foldable device with capacitive touch in unfolded state, on body with
 * Bluetooth active, balancing WiFi-Bluetooth coexistence while maintaining
 * touch sensitivity.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_BODY_CELL_ON_UNFOLDED_CAP: Power settings for
 * foldable device with capacitive touch in unfolded state, on body with
 * cellular radio enabled, managing WiFi-cellular coexistence while maintaining
 * touch sensitivity.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_BODY_CELL_ON_BT_UNFOLDED_CAP: Power settings for
 * foldable device with capacitive touch in unfolded state, on body with both
 * cellular and Bluetooth active, managing triple-radio coexistence while
 * maintaining touch sensitivity.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_BODY_CELL_OFF_CAP: Power settings for device
 * with capacitive touch on body with cellular radio disabled, optimizing WiFi
 * power in folded state while ensuring touch sensitivity is maintained.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_BODY_BT_CAP: Power settings for device with
 * capacitive touch on body with Bluetooth active, balancing WiFi-Bluetooth
 * coexistence in folded state while maintaining touch sensitivity.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_BODY_CELL_ON_CAP: Power settings for device with
 * capacitive touch on body with cellular radio enabled, managing WiFi-cellular
 * coexistence in folded state while maintaining touch sensitivity.
 *
 * @QCA_WIFI_POWER_SCENARIO_ON_BODY_CELL_ON_BT_CAP: Power settings for device
 ...
 * sensitivity.
 */
enum qca_wifi_power_scenarios {
	QCA_WIFI_POWER_SCENARIO_VOICE_CALL = 0,
	QCA_WIFI_POWER_SCENARIO_ON_HEAD_CELL_OFF = 1,
	QCA_WIFI_POWER_SCENARIO_ON_HEAD_CELL_ON = 2,
	QCA_WIFI_POWER_SCENARIO_ON_BODY_CELL_OFF = 3,
	QCA_WIFI_POWER_SCENARIO_ON_BODY_CELL_ON = 4,
	QCA_WIFI_POWER_SCENARIO_ON_BODY_BT = 5,
	QCA_WIFI_POWER_SCENARIO_ON_HEAD_HOTSPOT = 6,
	QCA_WIFI_POWER_SCENARIO_ON_HEAD_HOTSPOT_MMW = 7,
	QCA_WIFI_POWER_SCENARIO_ON_BODY_CELL_ON_BT = 8,
	QCA_WIFI_POWER_SCENARIO_ON_BODY_HOTSPOT = 9,
	QCA_WIFI_POWER_SCENARIO_ON_BODY_HOTSPOT_BT = 10,
	QCA_WIFI_POWER_SCENARIO_ON_BODY_HOTSPOT_MMW = 11,
	QCA_WIFI_POWER_SCENARIO_ON_BODY_HOTSPOT_BT_MMW = 12,
	QCA_WIFI_POWER_SCENARIO_ON_HEAD_CELL_OFF_UNFOLDED = 13,
	QCA_WIFI_POWER_SCENARIO_ON_HEAD_CELL_ON_UNFOLDED = 14,
	QCA_WIFI_POWER_SCENARIO_ON_HEAD_HOTSPOT_UNFOLDED = 15,
	QCA_WIFI_POWER_SCENARIO_ON_HEAD_HOTSPOT_MMW_UNFOLDED = 16,
	QCA_WIFI_POWER_SCENARIO_ON_BODY_CELL_OFF_UNFOLDED = 17,
	QCA_WIFI_POWER_SCENARIO_ON_BODY_BT_UNFOLDED = 18,
	QCA_WIFI_POWER_SCENARIO_ON_BODY_CELL_ON_UNFOLDED = 19,
	QCA_WIFI_POWER_SCENARIO_ON_BODY_CELL_ON_BT_UNFOLDED = 20,
	QCA_WIFI_POWER_SCENARIO_ON_BODY_HOTSPOT_UNFOLDED = 21,
	QCA_WIFI_POWER_SCENARIO_ON_BODY_HOTSPOT_BT_UNFOLDED = 22,
	QCA_WIFI_POWER_SCENARIO_ON_BODY_HOTSPOT_MMW_UNFOLDED = 23,
	QCA_WIFI_POWER_SCENARIO_ON_BODY_HOTSPOT_BT_MMW_UNFOLDED = 24,
	QCA_WIFI_POWER_SCENARIO_ON_BODY_REAR_CAMERA = 25,
	QCA_WIFI_POWER_SCENARIO_ON_BODY_CELL_OFF_UNFOLDED_CAP = 26,
	QCA_WIFI_POWER_SCENARIO_ON_BODY_BT_UNFOLDED_CAP = 27,
	QCA_WIFI_POWER_SCENARIO_ON_BODY_CELL_ON_UNFOLDED_CAP = 28,
	QCA_WIFI_POWER_SCENARIO_ON_BODY_CELL_ON_BT_UNFOLDED_CAP = 29,
	QCA_WIFI_POWER_SCENARIO_ON_BODY_CELL_OFF_CAP = 30,
	QCA_WIFI_POWER_SCENARIO_ON_BODY_BT_CAP = 31,
	QCA_WIFI_POWER_SCENARIO_ON_BODY_CELL_ON_CAP = 32,
	QCA_WIFI_POWER_SCENARIO_ON_BODY_CELL_ON_BT_CAP = 33,
};
 ...
 *  in enum %qca_wifi_power_scenario.
 *
 ...
 * @QCA_WLAN_VENDOR_CFR_LTF_TYPE_LEGACY: CSI from Legacy (IEEE 802.11a/g)
 */
...
enum qca_wlan_vendor_chip_id {
	...
	QCA_WLAN_VENDOR_CHIP_ID_WCN8750 = 15,
};
 ...
 * This is for CFR version 2 and version 3.
 *
 ...
 *
 * @QCA_WLAN_VENDOR_ATTR_DCS_CMD_TYPE: 8-bit unsigned value for DCS
 * command type
 *	0: GET – Retrieve the current DCS configuration.
 *	   User must provide QCA_WLAN_VENDOR_ATTR_DCS_CMD_TYPE.
 *	   QCA_WLAN_VENDOR_ATTR_DCS_LINK_ID is also required in multi-link SAP
 *	   scenarios.
 *	   Host will return the following attributes:
 ...
 *
 * @QCA_WLAN_VENDOR_ATTR_DCS_ENABLE: 16-bit bitmap to set/get
 * enable/disable DCS
 *	bit 0: Enable Continuous Wave Interference Management(CW IM)
 *	bit 1: Enable WLAN Interference Mitigation(WLAN IM)
 *
 * @QCA_WLAN_VENDOR_ATTR_DCS_INTERFERENCE_DETECTION_THRESHOLD: 32-bit
 * unsigned value to set/get interference detection threshold. This
 * attribute specifies the threshold for detecting interference that
 * triggers DCS channel change.
 *
 * @QCA_WLAN_VENDOR_ATTR_DCS_PHY_ERR_PENALTY: 32-bit unsigned value to
 * set/get the PHY error penalty. This value specifies the amount of
 * channel time penalized for each PHY error detected.
 *
 * @QCA_WLAN_VENDOR_ATTR_DCS_PHY_ERR_THRESHOLD: 32-bit unsigned value
 * to set/get the PHY error count threshold.
 *
 * @QCA_WLAN_VENDOR_ATTR_DCS_RADAR_ERR_THRESHOLD: 32-bit unsigned
 * value to set/get radar error count threshold.
 *
 * @QCA_WLAN_VENDOR_ATTR_DCS_TX_ERR_THRESHOLD: 32-bit unsigned value
 * to set/get TX error count threshold.
 *
 * @QCA_WLAN_VENDOR_ATTR_DCS_INTERFERENCE_DETECTION_WINDOW: 32-bit
 * unsigned value to set/get the interference detection sampling
 * window. The unit is a beacon interval.
 *
 * @QCA_WLAN_VENDOR_ATTR_DCS_COCHANNEL_INTERFERENCE_THRESHOLD: 8-bit
 * unsigned value to set/get the co-channel interference threshold
 * level, interpreted as a percentage (0-100).
 *
 * @QCA_WLAN_VENDOR_ATTR_DCS_MAX_CU: 8-bit unsigned value to set/get
 * the maximum channel utilization percentage allowed. If the combined
 * TX and RX channel utilization exceeds this threshold, DCS may
 * trigger a channel change.
 */
enum qca_wlan_vendor_attr_dcs {
