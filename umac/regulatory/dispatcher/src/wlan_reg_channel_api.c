                             ...
                             struct regulatory_channel *reg_chan_list)
{
	...
	for (i = 0; i < NUM_CHANNELS; i++) {
		...
		if (reg_chan_list[i].center_freq == freq2 &&
		    reg_chan_list[i].chan_flags & REGULATORY_CHAN_RADAR) {
