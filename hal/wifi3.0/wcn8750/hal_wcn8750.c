...
#include "hal_wcn8750_tx.h"
#include "hal_wcn8750_rx.h"
...
static uint32_t hal_get_link_desc_size_8750(void)
{
...
/**
 * hal_rx_dump_msdu_end_tlv_8750() - dump RX msdu_end TLV in structured
 ...
 * Return: void
 */
static void hal_rx_dump_msdu_end_tlv_8750(void *msduend,
					  uint8_t dbg_level)
{
...
/**
 * hal_rx_dump_mpdu_start_tlv_8750(): dump RX mpdu_start TLV in structured
 *			       human readable format.
 ...
 * Return: void
 */
static inline void hal_rx_dump_mpdu_start_tlv_8750(void *mpdustart,
						   uint8_t dbg_level)
{
...
/**
 * hal_rx_peer_meta_data_get_8750() - get peer meta data from rx_pkt_tlvs
 * @buf: start of rx_tlv_hdr
 *
 * Return: peer meta data
 */
static inline uint32_t hal_rx_peer_meta_data_get_8750(uint8_t *buf)
{
...
/**
 * hal_rx_mpdu_info_ampdu_flag_get_8750() - get ampdu flag bit
 * from rx mpdu info
 * @buf: pointer to rx_pkt_tlvs
 *
 * Return: ampdu flag
 */
static inline bool hal_rx_mpdu_info_ampdu_flag_get_8750(uint8_t *buf)
{
	struct rx_pkt_tlvs *rx_pkt_tlvs = (struct rx_pkt_tlvs *)buf;
...
/**
 * hal_rx_tlv_l3_type_get_8750() - API to get the l3 type from
 *                               rx_msdu_start TLV
 * @buf: pointer to the start of RX PKT TLV headers
 *
 * Return: uint32_t(l3 type)
 */
static inline uint32_t hal_rx_tlv_l3_type_get_8750(uint8_t *buf)
{
	struct rx_pkt_tlvs *rx_pkt_tlvs = (struct rx_pkt_tlvs *)buf;
...
#ifdef CONFIG_WORD_BASED_TLV
/**
 * hal_rx_priv_info_set_in_tlv_8750() - Save the private info to
 ...
 * Return: void
 */
static inline void hal_rx_priv_info_set_in_tlv_8750(uint8_t *buf,
						     uint8_t *priv_data,
						     uint32_t len)
{
...
/**
 * hal_rx_priv_info_get_from_tlv_8750() - retrieve the private data from
 *                             the reserved bytes of rx_tlv_hdr.
 * @buf: start of rx_tlv_hdr
 ...
 * Return: void
 */
static inline void hal_rx_priv_info_get_from_tlv_8750(uint8_t *buf,
						       uint8_t *priv_data,
						       uint32_t len)
{
	struct rx_pkt_tlvs *pkt_tlvs = (struct rx_pkt_tlvs *)buf;
...
/**
 * hal_rx_tlv_get_freq_8750() - API to get the frequency of operating
...
static inline uint32_t hal_rx_tlv_get_freq_8750(uint8_t *buf)
{
...
/**
 * hal_rx_mpdu_start_tlv_tag_valid_8750() - API to check if RX_MPDU_START
 *                                           tlv tag is valid
 * @rx_tlv_hdr: start address of rx_pkt_tlvs
 *
 * Return: true if RX_MPDU_START is valid, else false.
 */
static inline uint8_t hal_rx_mpdu_start_tlv_tag_valid_8750(void *rx_tlv_hdr)
{
...
static inline void hal_rx_dump_pkt_hdr_tlv_8750(struct rx_pkt_tlvs *pkt_tlvs,
						uint8_t dbg_level)
{
}

static inline
void hal_register_rx_pkt_hdr_tlv_api_8750(struct hal_soc *hal_soc)
{
}
...
/**
 * hal_rx_dump_pkt_hdr_tlv_8750() - dump RX pkt header TLV in hex format
...
/**
 * hal_register_rx_pkt_hdr_tlv_api_8750: register all rx_pkt_hdr_tlv related api
...
#ifdef CONFIG_WORD_BASED_TLV
/**
 * hal_rx_dump_pkt_tlvs_8750(): API to print RX Pkt TLVS for peach
 ...
 * Return: void
 */
static void hal_rx_dump_pkt_tlvs_8750(hal_soc_handle_t hal_soc_hdl,
				       uint8_t *buf, uint8_t dbg_level)
{
	...
	hal_rx_dump_msdu_end_tlv_8750(msdu_end, dbg_level);
	hal_rx_dump_mpdu_start_tlv_8750(mpdu_start, dbg_level);
	hal_rx_dump_pkt_hdr_tlv_8750(pkt_tlvs, dbg_level);
}
#else
static void hal_rx_dump_pkt_tlvs_8750(hal_soc_handle_t hal_soc_hdl,
				       uint8_t *buf, uint8_t dbg_level)
...
/**
 * hal_rx_get_tlv_8750() - API to get the tlv
 * @rx_tlv: TLV data extracted from the rx packet
 *
 * Return: uint8_t
 */
static uint8_t hal_rx_get_tlv_8750(void *rx_tlv)
{
	return HAL_RX_GET_64(rx_tlv, PHYRX_RSSI_LEGACY, RECEIVE_BANDWIDTH);
}

/**
 * hal_rx_phy_legacy_get_rssi_8750() - API to get RSSI from TLV
 *                                     WIFIPHYRX_RSSI_LEGACY_E
 * @buf: pointer to the start of WIFIPHYRX_RSSI_LEGACY_E TLV
 *
 * Return: value of RSSI
 */
static int8_t hal_rx_phy_legacy_get_rssi_8750(uint8_t *buf)
{
	return HAL_RX_GET_64(buf, PHYRX_RSSI_LEGACY, RSSI_COMB_PPDU);
}

/**
 * hal_reo_config_8750(): Set reo config parameters
 * @soc: hal soc handle
 ...
 */
static
void hal_reo_config_8750(struct hal_soc *soc,
			 uint32_t reg_val,
...
/**
 * hal_rx_msdu_desc_info_get_ptr_8750() - Get msdu desc info ptr
 * @msdu_details_ptr: Pointer to msdu_details_ptr
 *
 * Return: Pointer to rx_msdu_desc_info structure.
 *
 */
static void *hal_rx_msdu_desc_info_get_ptr_8750(void *msdu_details_ptr)
{
	return HAL_RX_MSDU_DESC_INFO_GET(msdu_details_ptr);
}

/**
 * hal_rx_link_desc_msdu0_ptr_8750() - Get pointer to rx_msdu details
 * @link_desc: Pointer to link desc
 *
 * Return: Pointer to rx_msdu_details structure
 *
 */
static void *hal_rx_link_desc_msdu0_ptr_8750(void *link_desc)
{
...
/**
 * hal_get_window_address_8750(): Function to get hp/tp address
 * @hal_soc: Pointer to hal_soc
 * @addr: address offset of register
 *
 * Return: modified address offset of register
 */
static inline qdf_iomem_t hal_get_window_address_8750(struct hal_soc *hal_soc,
						      qdf_iomem_t addr)
{
	uint32_t offset;
	uint32_t window;
...
/**
 * hal_reo_set_err_dst_remap_8750(): Function to set REO error destination
 *				     ring remap register
 * @hal_soc: Pointer to hal_soc
 *
 * Return: none.
 */
static void
hal_reo_set_err_dst_remap_8750(void *hal_soc)
{
	/*
...
/**
 * hal_rx_flow_setup_fse_8750() - Setup a flow search entry in HW FST
 ...
 */
static void *
hal_rx_flow_setup_fse_8750(uint8_t *rx_fst, uint32_t table_offset,
			   uint8_t *rx_flow)
{
...
/**
 * hal_rx_flow_setup_cmem_fse_8750() - Setup a flow search entry in HW CMEM FST
 ...
 */
static uint32_t
hal_rx_flow_setup_cmem_fse_8750(struct hal_soc *hal_soc, uint32_t cmem_ba,
...
/**
 * hal_rx_flow_get_cmem_fse_ts_8750() - Get timestamp field from CMEM FSE
 * @hal_soc: hal_soc reference
 * @fse_offset: CMEM FSE offset
 *
 * Return: Timestamp
 */
static uint32_t hal_rx_flow_get_cmem_fse_ts_8750(struct hal_soc *hal_soc,
						 uint32_t fse_offset)
{
...
/**
 * hal_rx_flow_get_cmem_fse_8750() - Get FSE from CMEM
 * @hal_soc: hal_soc reference
 * @fse_offset: CMEM FSE offset
 ...
 */
static void
hal_rx_flow_get_cmem_fse_8750(struct hal_soc *hal_soc, uint32_t fse_offset,
...
static
void hal_compute_reo_remap_ix2_ix3_8750(uint32_t *ring_map,
...
#define HAL_NUM_TCL_BANKS_WCN8750 8

/**
 * hal_tx_get_num_tcl_banks_8750() - Get number of banks in target
 *
 * Returns: number of bank
 */
static uint8_t hal_tx_get_num_tcl_banks_8750(void)
{
	return HAL_NUM_TCL_BANKS_WCN8750;
}

/**
 * hal_cmem_write_8750() - function for CMEM buffer writing
 ...
 * Return: None.
 */
static inline void hal_cmem_write_8750(hal_soc_handle_t hal_soc_hdl,
				       uint32_t offset,
				       uint32_t value)
{
...
/**
 * hal_get_idle_link_bm_id_8750() - Get idle link BM id from chid_id
 * @chip_id: mlo chip_id
 *
 * Returns: RBM ID
 */
static uint8_t hal_get_idle_link_bm_id_8750(uint8_t chip_id)
{
...
/**
 * hal_get_first_wow_wakeup_packet_8750(): Function to get if the buffer
 ...
 *	    WoW. Otherwise 0
 */
static inline uint8_t hal_get_first_wow_wakeup_packet_8750(uint8_t *buf)
{
	struct rx_pkt_tlvs *pkt_tlvs = (struct rx_pkt_tlvs *)buf;
...
static uint16_t hal_get_rx_max_ba_window_8750(int tid)
{
	return HAL_RX_BA_WINDOW_1024;
}

/**
 * hal_get_reo_qdesc_size_8750()- Get the reo queue descriptor size
 ...
 */
static uint32_t hal_get_reo_qdesc_size_8750(uint32_t ba_window_size, int tid)
{
	return 0;
}
...
/**
 * hal_get_tsf_time_8750() - Get tsf time from scratch register
 * @hal_soc_hdl: HAL soc handle
 * @tsf_id: TSF id
 * @mac_id: mac_id
 ...
 */
static void
hal_get_tsf_time_8750(hal_soc_handle_t hal_soc_hdl, uint32_t tsf_id,
...
static QDF_STATUS hal_rx_reo_ent_get_src_link_id_8750(hal_rxdma_desc_t rx_desc,
						      uint8_t *src_link_id)
{
...
/**
 * hal_rx_en_mcast_fp_data_filter_8750() - Is mcast filter pass enabled
 *
 * Return: false for BE MCC
 */
static inline
bool hal_rx_en_mcast_fp_data_filter_8750(void)
{
	return false;
}

/**
 * hal_srng_dst_hw_init_misc_1_8750() - Function to initialize MISC_1 register
 ...
 */
static inline
void hal_srng_dst_hw_init_misc_1_8750(struct hal_srng *srng)
{
	uint32_t reg_val = 0;
...
/**
 * hal_srng_hw_reg_offset_init_misc_1_8750() - Initialize the HW srng register
 *                                             offset of MISC_1
 ...
 */
static inline
void hal_srng_hw_reg_offset_init_misc_1_8750(struct hal_soc *hal_soc)
{
...
/**
 * hal_srng_dst_hw_init_8750() - Function to initialize SRNG
 *                               destination ring HW
 * @hal_soc: HAL SOC handle
 * @srng: SRNG ring pointer
 ...
 */
static inline
void hal_srng_dst_hw_init_8750(struct hal_soc *hal_soc,
			       struct hal_srng *srng,
			       bool idle_check,
			       uint32_t idx)
{
	hal_srng_dst_hw_init_misc_1_8750(srng);
...
static void hal_hw_txrx_ops_attach_8750(struct hal_soc *hal_soc)
{
	/* init and setup */
	hal_soc->ops->hal_srng_dst_hw_init = hal_srng_dst_hw_init_8750;
	hal_soc->ops->hal_srng_src_hw_init = hal_srng_src_hw_init_generic;
	hal_soc->ops->hal_get_hw_hptp = hal_get_hw_hptp_generic;
	hal_soc->ops->hal_get_window_address = hal_get_window_address_8750;
	hal_soc->ops->hal_reo_set_err_dst_remap =
						hal_reo_set_err_dst_remap_8750;
	/* Overwrite the default BE ops */
	hal_soc->ops->hal_get_rx_max_ba_window = hal_get_rx_max_ba_window_8750;
	hal_soc->ops->hal_get_reo_qdesc_size = hal_get_reo_qdesc_size_8750;

	/* tx */
	hal_soc->ops->hal_tx_set_dscp_tid_map = hal_tx_set_dscp_tid_map_8750;
	hal_soc->ops->hal_tx_update_dscp_tid = hal_tx_update_dscp_tid_8750;
	hal_soc->ops->hal_tx_comp_get_status =
					hal_tx_comp_get_status_generic_be;
	hal_soc->ops->hal_tx_init_cmd_credit_ring =
					hal_tx_init_cmd_credit_ring_8750;
	hal_soc->ops->hal_tx_config_rbm_mapping_be =
				hal_tx_config_rbm_mapping_be_8750;
	...
	hal_soc->ops->hal_rx_get_tlv = hal_rx_get_tlv_8750;
	hal_soc->ops->hal_rx_pkt_hdr_get = hal_rx_pkt_hdr_get_be;

	hal_soc->ops->hal_rx_dump_msdu_end_tlv = hal_rx_dump_msdu_end_tlv_8750;
	hal_soc->ops->hal_rx_dump_mpdu_start_tlv =
					hal_rx_dump_mpdu_start_tlv_8750;
	hal_soc->ops->hal_rx_dump_pkt_tlvs = hal_rx_dump_pkt_tlvs_8750;
	hal_soc->ops->hal_rx_desc_get_80211_hdr = hal_rx_desc_get_80211_hdr_be;

	hal_soc->ops->hal_get_link_desc_size = hal_get_link_desc_size_8750;
	...
	hal_soc->ops->hal_rx_msdu_desc_info_get_ptr =
					hal_rx_msdu_desc_info_get_ptr_8750;
	hal_soc->ops->hal_rx_link_desc_msdu0_ptr =
					hal_rx_link_desc_msdu0_ptr_8750;
	...
	hal_soc->ops->hal_rx_priv_info_set_in_tlv =
					hal_rx_priv_info_set_in_tlv_8750;
	hal_soc->ops->hal_rx_priv_info_get_from_tlv =
					hal_rx_priv_info_get_from_tlv_8750;
	...
	hal_soc->ops->hal_rx_mpdu_start_tlv_tag_valid =
					hal_rx_mpdu_start_tlv_tag_valid_8750;
#else
	hal_soc->ops->hal_rx_priv_info_set_in_tlv =
	...
	hal_soc->ops->hal_rx_tlv_peer_meta_data_get =
		hal_rx_peer_meta_data_get_8750;
	...
	hal_soc->ops->hal_rx_phy_legacy_get_rssi =
					hal_rx_phy_legacy_get_rssi_8750;
	...
	hal_soc->ops->hal_reo_config = hal_reo_config_8750;
...
#if defined(QCA_WIFI_WCN8750) && defined(WLAN_CFR_ENABLE) && \
	defined(WLAN_ENH_CFR_ENABLE)
	hal_soc->ops->hal_rx_get_bb_info = hal_rx_get_bb_info_8750;
	hal_soc->ops->hal_rx_get_rtt_info = hal_rx_get_rtt_info_8750;
#else
	...
	hal_soc->ops->hal_compute_reo_remap_ix2_ix3 =
					hal_compute_reo_remap_ix2_ix3_8750;
	hal_soc->ops->hal_rx_flow_setup_cmem_fse =
						hal_rx_flow_setup_cmem_fse_8750;
	hal_soc->ops->hal_rx_flow_get_cmem_fse_ts =
					hal_rx_flow_get_cmem_fse_ts_8750;
	hal_soc->ops->hal_rx_flow_get_cmem_fse = hal_rx_flow_get_cmem_fse_8750;
	hal_soc->ops->hal_cmem_write = hal_cmem_write_8750;
	hal_soc->ops->hal_rx_msdu_get_reo_destination_indication =
		hal_rx_msdu_get_reo_destination_indication_be;
	hal_soc->ops->hal_tx_get_num_tcl_banks = hal_tx_get_num_tcl_banks_8750;
	...
	hal_soc->ops->hal_rx_tlv_get_freq = hal_rx_tlv_get_freq_8750;
	...
	hal_soc->ops->hal_rx_mpdu_info_ampdu_flag_get =
					hal_rx_mpdu_info_ampdu_flag_get_8750;
	hal_soc->ops->hal_rx_tlv_msdu_len_set =
					hal_rx_msdu_start_msdu_len_set_be;
	hal_soc->ops->hal_get_idle_link_bm_id = hal_get_idle_link_bm_id_8750;
#ifdef WLAN_FEATURE_MARK_FIRST_WAKEUP_PACKET
	hal_soc->ops->hal_get_first_wow_wakeup_packet =
		hal_get_first_wow_wakeup_packet_8750;
#endif
	hal_soc->ops->hal_compute_reo_remap_ix0 = NULL;

	hal_soc->ops->hal_rx_tlv_l3_type_get = hal_rx_tlv_l3_type_get_8750;
	...
	hal_soc->ops->hal_get_tsf_time = hal_get_tsf_time_8750;
	hal_soc->ops->hal_rx_reo_ent_get_src_link_id =
					hal_rx_reo_ent_get_src_link_id_8750;
	...
	hal_soc->ops->hal_rx_en_mcast_fp_data_filter =
					hal_rx_en_mcast_fp_data_filter_8750;
...
/**
 * hal_srng_hw_reg_offset_init_8750() - Initialize the HW srng reg offset
 *				applicable only for peach
 * @hal_soc: HAL Soc handle
 *
 * Return: None
 */
static inline void hal_srng_hw_reg_offset_init_8750(struct hal_soc *hal_soc)
{
	int32_t *hw_reg_offset = hal_soc->hal_hw_reg_offset;
	...
	hal_srng_hw_reg_offset_init_misc_1_8750(hal_soc);
}

void hal_wcn8750_attach(struct hal_soc *hal_soc)
{
	hal_soc->hw_srng_table = hw_srng_table_8750;

	hal_srng_hw_reg_offset_init_generic(hal_soc);
	hal_srng_hw_reg_offset_init_8750(hal_soc);
	hal_hw_txrx_default_ops_attach_be(hal_soc);
	hal_hw_txrx_ops_attach_8750(hal_soc);
}
