...
/**
 * dp_tx_gen_hw_desc_be() - Generate HW descriptor without HW enqueue
 * @soc: DP Soc Handle
 * @vdev: DP vdev handle
 * @tx_desc: Tx Descriptor Handle
 * @fw_metadata: Metadata to send to Target Firmware along with frame
 ...
 */
QDF_STATUS dp_tx_gen_hw_desc_be(struct dp_soc *soc,
				struct dp_vdev *vdev,
				struct dp_tx_desc_s *tx_desc,
				uint16_t fw_metadata,
				struct cdp_tx_exception_metadata *metadata,
...
/**
 * dp_tx_desc_update_buffer_info_bn() - Update buffer info in HW descriptor
 * @soc: DP soc handle
 * @hal_tx_desc_cached: cached descriptor in host memory
 ...
 * Return: None
 */
void dp_tx_desc_update_buffer_info_bn(struct dp_soc *soc,
				      void *hal_tx_desc_cached,
				      struct dp_tx_desc_s *tx_desc,
				      uint8_t ring_id);

/**
 * dp_tx_desc_update_buffer_info_be_bn() - Update buffer info for BE/BN target
 * @soc: DP soc handle
 * @hal_tx_desc_cached: cached descriptor in host memory
 * @tx_desc: tx descriptor
 * @ring_id: ring ID
 ...
 */
static inline
void dp_tx_desc_update_buffer_info_be_bn(struct dp_soc *soc,
					 void *hal_tx_desc_cached,
					 struct dp_tx_desc_s *tx_desc,
					 uint8_t ring_id)
{
	dp_tx_desc_update_buffer_info_bn(soc, hal_tx_desc_cached, tx_desc,
					 ring_id);
}
#else /* CONFIG_BORON */
/**
